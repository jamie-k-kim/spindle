...
BUA := function(s, ruleset, opts)
    local cx, max_iter, iter;
    cx := empty_cx();
    cx.opts := opts;
    cx.applied := 1;
    max_iter := 100;  # hard cap: prevents infinite cycling
    iter := 0;
    while cx.applied > 0 and iter < max_iter do
        cx.applied := 0;
        s := _ApplyAllRulesBottomUp(s, cx, ruleset);
        iter := iter + 1;
    od;
    if iter = max_iter then
        Print("Warning: BUA hit iteration cap. Returning best result so far.\n");
    fi;
    return s;
end;
