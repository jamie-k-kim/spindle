    ...
    **kwargs : dict
    ...
    def kernel(self, mf, fragmentation: str = "atomic", **kwargs):
        ...
        **kwargs : dict
        ...
        valid_frag = ["atomic", "iao", "site", "orbital"]
        ...
        elif fragmentation.lower() == "orbital":
            # Orbital fragmentation for abstract orbitals (e.g., from FCIDUMP)
            with self._vayesta_ewf.cas_fragmentation() as f:
                if atom_indices is not None:
                    for orb_idx in atom_indices:
                        f.add_orbital_fragment([orb_idx])
                else:
                    for orb_idx in range(mf.mol.nao):
                        f.add_orbital_fragment([orb_idx])
        else:
            raise ValueError(
                f"Unknown fragmentation scheme: {fragmentation}. " "Use 'atomic', 'iao', 'site', or 'orbital'."
            )
